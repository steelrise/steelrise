...
function _default() {
    // prerun checks
    if (!document.querySelector('.header-1')) return; // store common references
    ...
    var opacity,
        ...
        scheduled = false; // add a platform state flag when in large viewports (desktop)
    ...
    // check to see if we should remove the compact menu styles and display the original menu
        ...
        !function fade() {
            if ((opacity += .1) <= 1) {
                ...
                requestAnimationFrame(fade); // clean up animation artifacts:
    ...
    function fadeOut(currentState) {
        ...
        !function fade() {
            if ((opacity -= .1) < 0) {
                menu.style.display = 'none'; // we're transitioning state, add a class to display the compact version of the menu
    ...
    // trivial debouncing
        if (!scheduled) {
            scheduled = true;
            setTimeout(function() {
                scheduled = false; // fade out expanded menu style
    ...
    window.addEventListener('resize', function() {
        var initialWidth = window.innerWidth; // trivial debouncing

        if (!scheduled) {
            scheduled = true;
            setTimeout(function() {
                scheduled = false; // change the platform state flag when resizing into mobile

                if (initialWidth <= compactMenuResizeTrigger && header.getAttribute('data-platform') === platformLarge) {
                    header.setAttribute('data-platform', platformSmall);
                } else if ( // if we come from a mobile state, and our viewport width is greater than the
                // maximum for mobile, and we have not yet scrolled beyond the trigger point for
                // activating the compact menu on desktop
                    header.getAttribute('data-platform') === platformSmall && initialWidth >= compactMenuResizeTrigger && window.scrollY <= compactMenuScrollTrigger) {
                    // show expanded menu
                    fadeIn('expanded'); // reset state

                    header.setAttribute('data-platform', platformLarge);
                } else if ( // we are scrolled beyond the trigger point for the compact menu, and have
                // resized back into desktop width; reset state but do not restore expanded menu
                    header.getAttribute('data-platform') === platformSmall && initialWidth >= compactMenuResizeTrigger) {
                    header.setAttribute('data-platform', platformLarge);
                }
            }, 250);
        }
